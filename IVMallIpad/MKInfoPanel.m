...
+(MKInfoPanel *)showPanelInView:(UIView *)view title:(NSString *)title isMiddleShow:(BOOL )showPlace {
    ...
    if (showPlace) {
        panel.frame = CGRectMake(0, 200, view.bounds.size.width, panelHeight);
    }
    else
    {
...
//    [Commonality setImgViewStyle:panel];
...
-(void)hidePanel {
    if ( [self.detailLabel.text isEqualToString:@"showMiddle"]) {
        ...
        self.frame = CGRectMake(0, self.frame.size.height* 100, self.frame.size.width, self.frame.size.height);
