...
//    UIImageView * imgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"small_logo.png"]];
//    imgView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
//    [self addSubview:_imgView1 = imgView];
...
- (void)setFrame:(CGRect)frame
{
    ...
    _statusLabel.frame = CGRectMake(statusX, statusY + 5, statusWidth, statusHeight);
    
    // 2.时间标签
    CGFloat lastUpdateY = statusY + statusHeight + 3;
    _lastUpdateTimeLabel.frame = CGRectMake(statusX, lastUpdateY, statusWidth, statusHeight);
    
    // 3.箭头
    CGFloat arrowX = w * 0.5 - 100;
    _arrowImage.center = CGPointMake(arrowX, frame.size.height * 0.8);
    
    // 4.指示器
    _activityView.center = _arrowImage.center;
    
    _imgView1.frame = CGRectMake(statusX, statusY, 158, 40);
    _imgView1.center = CGPointMake(160, statusY-15);
}
